...
bool findNearestNodeToCoordinate(point_t coordinate, node_t *list_of_nodes, int num_of_nodes, node_t **nearest_node, int world_rank, int world_size){
    ...
    for(int i = world_rank; i < num_of_nodes; i += world_size) {
    ...
    int root = 0;
    int min_d2_array[world_size];
    int min_d2_index_array[world_size];
    MPI_Gather(&min_d2,  1, MPI_INT, 
               min_d2_array, 1, MPI_INT,
               0, MPI_COMM_WORLD);
    MPI_Gather(&min_d2_index,  1, MPI_INT, 
               min_d2_index_array, 1, MPI_INT,
                0, MPI_COMM_WORLD);
...
if(world_rank==0){
    ...
    MPI_Bcast (&min_d2, 1, MPI_INT, 0, MPI_COMM_WORLD);
    // printf("min_index(%d): %d\n", world_rank, min_index);
    // printf("min_d2(%d): %d\n", world_rank, min_d2);
...
int main(int argc, const char *argv[]) {    
    ...
    // MIP --- --------
    MPI_Init(&_argc, const_cast<char***>(&_argv));
    int world_rank, world_size;

    // TODO: MPI init
    MPI_Comm_size(MPI_COMM_WORLD, &world_size); // total processes
    MPI_Comm_rank(MPI_COMM_WORLD, &world_rank); // which process
...
if(world_rank==0){
    ...
    for(num_nodes_generated = 1; num_nodes_generated < max_num_of_nodes; num_nodes_generated++){
        ...
        do{
            ...
            //     MPI_Send(&random_point, sizeof(point_t), MPI_BYTE, i, 0, MPI_COMM_WORLD);
            // }
